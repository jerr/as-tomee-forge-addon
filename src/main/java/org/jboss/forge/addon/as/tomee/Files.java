...
public class Files {
    ...
    public static boolean extractAppServer(final String zipPath, final File target, final boolean overwrite) throws IOException {
        if (target.exists() && !overwrite) {
            throw new IllegalStateException("Not empty directory");
        }
        // Create a temporary directory
        final File tmpDir = new File(getTempDirectory(), "tomee-" + zipPath.hashCode());
        if (tmpDir.exists()) {
        ...
        try {
            ...
            try {
                ...
                throw new IOException("Error extracting '" +  file + "'.", e);
            } finally {
